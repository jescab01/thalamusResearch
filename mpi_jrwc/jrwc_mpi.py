...
coupling_jr = np.arange(0, 120, 1)
coupling_wc = np.logspace(-8, 1, 50)
# coupling_wc = np.arange(0, 0.01, 0.0005)
noise_vals = [0.15]  # 0

params = [[subj, th, cer, g_jr, g_wc, std_n, r] for subj in subjects
          for th in structure_th for cer in structure_cer
          for g_jr in coupling_jr for g_wc in coupling_wc for std_n in noise_vals
...
local_results = jrwc_parallel(local_params)  # run the function for each parameter set and rank
...
if rank > 0:  # WORKERS _send to rank 0
    ...
    fResults_df = pd.DataFrame(final_results, columns=["subject", "th", "cer", "g_jr", "g_wc", "std_n", "rep",
