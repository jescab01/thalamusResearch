...
execute in terminal with : mpiexec -n 2 python mpi_thcer.py
"""
...
coupling_vals = np.arange(0, 60, 1)  # 0.5
speed_vals = list(np.arange(0.5, 25, 1)) + list(np.arange(2, 4, 0.25))
noiseth_vals = [0.022]  #define valor  # [2.2e-8, , 0.022]  # np.logspace(-8, 2, 30)
n_rep = 3

        # [subj, "jr", th, "pCer", g, s, 0.09, 0.022, 0.09, 2.2e-8, r] orig
params = [[subj, "jr", th, "pCer", g, s, 0.15, 0.09, 0.09, 2.2e-8, r] for subj in subjects
          for th in structure_th for s in speed_vals for g in coupling_vals for r in range(n_rep)]
...
if rank > 0:  # WORKERS _send to rank 0
    ...
    fResults_df = pd.DataFrame(final_results, columns=["subject", "model", "th", "cer", "g", "s",
                                                       "pth", "sigmath", "pcx", "sigmacx", "rep",
                                                       "min_cx", "max_cx", "min_th", "max_th",
                                                       "IAF", "module", "bModule", "band",
                                                       "rPLV", "dFC_KSD", "KOstd", "KOstd_emp", "sc_r"])
