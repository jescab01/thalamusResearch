...
noise_vals = [0] + list(np.logspace(-4, 2, 60))  # np.arange(0.05, 0.5, 0.01)  #   # [0, 0.022]  # np.linspace(0, 0.1, 40)  # 20
n_rep = 3

params = [[subj, model, th, cer, g, 0.15, sigmath, 0.09, 2.2e-8, r] for subj in subjects
          for model in models for th in structure_th for cer in structure_cer
          for g in coupling_vals for sigmath in noise_vals for r in range(n_rep)]
