...
left) and the delayed state vector. This order is important in the case of an asymmetric
connectivity matrix, where the convention to distinguish target ($k$) and source ($j$)
...
class SigmoidalJansenRit_Linear(Coupling):
    ...
    def pre(self, x_i, x_j):
        ...
        pre[:, 0, jr_mask, :] = self.e0 / (1.0 + numpy.exp(self.r * (self.v0 - sum_vExc_vInh[:, jr_mask, numpy.newaxis])))
...
class SigmoidalJansenRitDavid(Coupling):
    ...
    def __str__(self):
        return simple_gen_astr(self, 'e0 v0 a r')

    def pre(self, x_i, x_j):
        # Modified on 11-Oct-2021: numpy.squeeze so that a vector of shape (22,1) can be multiplied to a matrix (22, 22)
        # when w is a heterogeneous parameter. Before squeezing matrix is (22, 22, 1) which is not possible to multiply.
        if len(self.w) > 1:
            sum_vExc_vInh = self.w * numpy.squeeze((x_j[:, 0] - x_j[:, 1])) + (1-self.w) * numpy.squeeze((x_j[:, 2] - x_j[:, 3]))
            pre = self.e0 / (1.0 + numpy.exp(self.r * (self.v0 - sum_vExc_vInh[:, :, numpy.newaxis])))
        else:
            sum_vExc_vInh = self.w *(x_j[:, 0] - x_j[:, 1]) + (1-self.w) * (x_j[:, 2] - x_j[:, 3])
