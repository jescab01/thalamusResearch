...
##       FIGURE     ##################
n_g = len(g_sel)
specs = [[{}, {"colspan": n_g, "secondary_y": True}] + [{}]*(n_g-1)] + [[{} for g in range(n_g+1)]]*4

id_emp = (n_g + 1) * 3 - 1
sp_titles = ["Empirical" if i == id_emp else "" for i in range((n_g+1)*5)]
sp_titles_r1 = [param + "==" + str(g) for g in g_sel]
sp_titles[6:6+n_g-1] = sp_titles_r1


fig = make_subplots(rows=5, cols=n_g+1, specs=specs, row_titles=["", "signals", "FFT", "FC", "dFC"],
...
for col in range(n_g+1):  # +1 empirical column
    ...
    # fourth row
    idx = 3 * (n_g+1) + (col+1) + 2 # +1 to avoid 0 indexing in python
    ...
    # fifth row
    idx = 4 * (n_g+1) + (col+1)  # +1 to avoid 0 indexing in python
    fig["layout"]["xaxis" + str(idx)]["title"] = {'text': 'Time (s)'}
    if idx == (4 * (n_g+1) + 2):
